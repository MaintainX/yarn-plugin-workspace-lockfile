import { Descriptor, MessageName, Plugin, Project, Report, structUtils } from "@yarnpkg/core";
  ...
  hooks: {
    async afterAllInstalled(project: Project, opts: { report: Report }) {
      ...
      for (const workspace of workspaces) {
        ...
        opts.report.reportInfo(MessageName.UNNAMED, `Generating lockfile for ${workspaceName}...`);
        ...
        try {
          ...
          for (const depType of ["dependencies", "devDependencies", "peerDependencies"] as const) {
            ...
            opts.report.reportInfo(MessageName.UNNAMED, `Found ${deps.size} ${depType}`);
            ...
            for (const dep of deps.values()) {
              ...
              if (workspaceDep) {
                ...
                const descriptor = structUtils.makeDescriptor(
                  structUtils.makeIdent(dep.scope || "", dep.name),
                  dep.range,
                );
                ...
                opts.report.reportInfo(
                  MessageName.UNNAMED,
                  `Added dependency: ${structUtils.stringifyDescriptor(descriptor)}`,
                );
              }
            }
          }
          ...
          const processDependency = (descriptor: Descriptor) => {
            ...
            if (!resolution) {
              ...
              if (!pkgKey) {
                opts.report.reportInfo(MessageName.UNNAMED, `No resolution found for ${descriptorStr}`);
                return;
              }

              const pkg = project.storedPackages.get(pkgKey);
              if (!pkg) {
                opts.report.reportInfo(MessageName.UNNAMED, `No package found for ${descriptorStr}`);
                return;
              }
          ...
          opts.report.reportInfo(
            MessageName.UNNAMED,
            `Generated ${workspaceLockfile.size} entries for workspace lockfile`,
          );
          ...
          opts.report.reportInfo(MessageName.UNNAMED, `Created ${workspaceLockfilePath}`);
        } catch (error: unknown) {
          const errorMessage = error instanceof Error ? error.message : String(error);
          opts.report.reportError(
            MessageName.UNNAMED,
            `Failed to generate lockfile for ${workspaceName}: ${errorMessage}`,
          );
        }
      }
    },
